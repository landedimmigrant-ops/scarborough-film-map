...
| 2 | 🔴 **Fix two-column field labels.** `Status / Type / Shoot date / Best light / Permit / Parking` don't get the small-uppercase label style (the `.field > label` direct-child selector skips nested labels), so they look like a different, louder heading than `TITLE / ADDRESS`. | Consistency & standards | Med | Low | proposed | C1 |
...
focus-visible rings, which landed with #13. Desktop layout unchanged. **Remaining:** #5, #7, #2,
#4, #6, #11, #12, #15, #8 (none High-impact).
